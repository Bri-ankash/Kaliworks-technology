...
export default function Reviews(){
  ...
  return(
    <section id="reviews" style={{background:#000000,borderTop:'1px solid #000000}}>
      <div style={{maxWidth:900,margin:'0 auto'}}>
        ...
        {/* Leave a review */}
        <div className="reveal" style={{background:#000000,border:'1px solid #000000,borderRadius:12,padding:24,marginBottom:16}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}
                  ...
                  style={{background:#000000,border:'1px solid #000000,borderRadius:8,padding:'10px 14px',color:'#fff',fontSize:13,outline:'none'}}
              ...
              <textarea value={form.text} onChange={e=>setForm(f=>({...f,text:e.target.value}))}
                ...
                style={{background:#000000,border:'1px solid #000000,borderRadius:8,padding:'10px 14px',color:'#fff',fontSize:13,outline:'none',resize:'vertical'}}
                onFocus={e=>e.target.style.borderColor='#84cc16'}
                onBlur={e=>e.target.style.borderColor=#000000}/>
              <button onClick={submitReview} disabled={submitting||!form.name.trim()||!form.text.trim()}
                style={{alignSelf:'flex-start',padding:'10px 24px',borderRadius:8,background:'#84cc16',color:'#292121',fontWeight:700,fontSize:13,border:'none',cursor:'pointer',opacity:submitting?0.7:1}}>
                Submit Review
              </button>
            </div>
          )}
        ...
        <div className="reveal">
          ...
          {adminInput&&(
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              <button onClick={checkAdmin} style={{background:'#84cc16',color:'#292121',border:'none',borderRadius:6,padding:'6px 12px',fontSize:12,fontWeight:700,cursor:'pointer'}}>Enter</button>
            </div>
          )}
          {adminMode&&(
