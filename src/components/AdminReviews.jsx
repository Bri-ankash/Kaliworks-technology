    ...
    <div style={{minHeight:'100vh',background:'var(--black)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{background:'var(--card)',border:'1px solid var(--border2)',borderRadius:20,padding:40,width:'100%',maxWidth:360,textAlign:'center'}}>
        ...
        <button onClick={()=>{ if(pwd===ADMIN_PASSWORD) setAuth(true); else alert('Wrong password!') }}
          style={{width:'100%',background:'var(--blue)',color:'#000',padding:12,borderRadius:10,fontWeight:700,fontSize:14,border:'none',cursor:'pointer'}}>
          Login
        </button>
      </div>
    </div>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
                {filter !== 'approved' && (
                  <button onClick={()=>updateStatus(r.id,'approved')}
                    style={{background:'#22c55e',color:'#000',padding:'8px 18px',borderRadius:8,fontWeight:700,fontSize:13,border:'none',cursor:'pointer'}}>
                    ✓ Approve
                  </button>
                )}
                {filter !== 'rejected' && (
                  <button onClick={()=>updateStatus(r.id,'rejected')}
                    style={{background:'#f59e0b',color:'#000',padding:'8px 18px',borderRadius:8,fontWeight:700,fontSize:13,border:'none',cursor:'pointer'}}>
                    ✗ Reject
                  </button>
                )}
